...
November 2014 version 0.3  by Klaas van Rijt.
    1) Added function: get_ancestors(layer)
    2) Changed layer selection combo box to show layer numbers + names in tree format.
       In addition set selection default to active layer.
    3) Set the active layer to the selection in the combo box.
...
class LayerViewer(gtk.Window):
    def __init__ (self, img, drw, *args):
        ...
        for i in range(max_pos):
            if layers[i] == self.img.active_layer:
                combo_active = i
            indent = ''
            geneo = get_ancestors(layers[i])
            if len(geneo) > 0:
                indent = '.\t'*len(geneo)

            self.combo_box.append_text(_('#{:4d}:\t'.format(i+1) + indent + layers[i].name))
        ...
        self.combo_box.set_active(combo_active)
        return r
    ...
    def name_change(self, btn, data=None) :
        ...
        if same_layers :
            ...
            self.img.active_layer = layers[index]
            #> layer offsets
...
def get_ancestors(child):
    """
    Get all the ancestors of a layer.
    Returns a list of successive parents.
    The list is empty when the layer is at root level.
    """
    container=[]
    if child.parent:
        container.append(child.parent)
        container.extend(get_ancestors(child.parent) )
    return container
